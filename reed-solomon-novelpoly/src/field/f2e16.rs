...
unsafe fn unpack_u16x8(v: u16x8) -> [u16; 8] {
	...
	_mm_store_si128(
		core::intrinsics::transmute((&mut dest.data) as *mut [u16;8]),
		v
	);
...
impl Additive8x {
	...
	pub fn mul(&self, other: Multiplier) -> Self {
		...
		unsafe {
			...
			let logtable = _mm256_castsi128_si256(logtable);
			let log = _mm256_add_epi16(logtable, other);
			
			// (log & ONEMASK) + (log >> shift)
			let onemasks = splat_u32x8(ONEMASK as Wide);
			let offset_lo = _mm256_and_si256(log, onemasks);
			let offset_hi = _mm256_srli_epi32(log, FIELD_BITS as i32);
			let offset = _mm256_add_epi32(offset_lo, offset_hi);
	...
	pub fn load(data: &[Additive]) -> Self {
		...
		let v = unsafe {
			_mm_loadu_si128(core::intrinsics::transmute(data.as_ptr()))
		};
		Self(v)
	}
...
impl Into<[Additive;8]> for Additive8x {
	fn into(self) -> [Additive;8] {
		unsafe {
			std::intrinsics::transmute(unpack_u16x8(self.0))
		}
	}
}
