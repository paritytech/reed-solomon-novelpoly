	...
	while depart_no < size {
		...
		while j < size {
			for i in (j..(j + depart_no)).step_by(LANE) {
				...
				unsafe {
					let data0 =
						_mm_loadu_si128(core::intrinsics::transmute::<_, *const u16x8>(data[i..][..LANE].as_ptr()));
					// dbg!(unpack_u16x8(data0));

					let data1 = _mm_loadu_si128(core::intrinsics::transmute::<_, *const u16x8>(
						data[(i + depart_no)..][..LANE].as_ptr(),
					));
...
fn walsh_output_plain_eq_faster8() {
	...
	{
		let data = unsafe { core::mem::transmute::<_, &mut [u16; SZ]>(&mut data) };
		rng.fill(&mut data[..]);
	}
	let mut d0 = data;
	let mut d1 = data;
