...
fn bench_status_quo_roundtrip() {
    roundtrip(status_quo_encode, status_quo_reconstruct, black_box(BYTES));
}

fn bench_status_quo_encode() {
    let _ = status_quo_encode(black_box(BYTES));
}

// TODO generate static data with `build.rs` for a decode run
